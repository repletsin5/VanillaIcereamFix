...
public class VanillaFixLoadingPlugin implements IFMLLoadingPlugin {
    ...
    static {
        ...
        config = new LoadingConfig(new File(Launch.minecraftHome, "config/vanillaicecreamfix.cfg"));
    ...
    private static void initMixin() {
        MixinBootstrap.init();
        boolean lolasm = false;
        try {
            lolasm =  Launch.classLoader.getClassBytes("zone.rong.loliasm.LoliASM") !=null;
        ...
        if(lolasm !=true){
            ...
            log.info("Not Loading Crash GUI and  as LoliASM/CensoredASM is loaded");
            log.info("Not Stacktrace Deobfuscator as LoliASM/CensoredASM is loaded");
        }
        ...
        if (config.profiler) {
            ...
            log.info("Not using Profiler Improvement Fix Mixins");
        }
        if (config.textureFixes) {
            ...
            log.info("Not using Texture Fix Mixins");
        }
